...
Grade: {grade}
Date: {meeting_date}
Duration: {agenda.get('duration', '30 minutes')}
...
"""
        ...
        if 'agenda_items' in agenda:
            for i, item in enumerate(agenda['agenda_items'], 1):
                agenda_text += f"""
{i}. {item.get('topic', 'Discussion Item')} ({item.get('time', '5 minutes')})
...
MEETING GOALS:
=============
1. Celebrate {first_name}'s strengths and achievements
